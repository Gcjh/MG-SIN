...
def evaluate(epoch, model, criterion, dataloader, device):
    model.eval()
    mean_loss, count =  0, 0
    mean_acc0, mean_acc1 = 0, 0
    n_pre0, n_labels0 = None, None
    n_pre1, n_labels1 = None, None
    ST_atten = '_ST_atten.pkl'
    SA_atten = '_SA_atten.pkl'
    ST_graph = '_ST_graph.pkl'
    SA_graph = '_SA_graph.pkl'
    with torch.no_grad():
        for input_ids, attention_mask, types, labels, graphs, target_len, tokens in tqdm(dataloader, desc="Evaluating"):
            ...
            logits0, logits1, ls, lh, atten = model(inputs, masks, graphs, target_len)

            for i in range(atten[0].shape[0]):
                with open('./attention/'+str(epoch)+'/'+str(count*16+i)+ST_atten, 'wb') as f:
                    pk.dump(atten[0][i], f)
                with open('./attention/'+str(epoch)+'/'+str(count*16+i)+ST_graph, 'wb') as f:
                    pk.dump(graphs[0][i], f)
                    
                with open('./attention/'+str(epoch)+'/'+str(count*16+i)+SA_atten, 'wb') as f:
                    pk.dump(atten[1][i], f)
                with open('./attention/'+str(epoch)+'/'+str(count*16+i)+SA_graph, 'wb') as f:
                    pk.dump(graphs[1][i], f)
